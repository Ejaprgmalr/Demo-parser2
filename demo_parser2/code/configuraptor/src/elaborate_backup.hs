...
type REnv = Map RName (Int, Int, Int)
type REnvUnit = (RName, (Int, Int, Int))
...
buildResourceMap ((ckind, rn, i):rest) container = 
    case Map.lookup rn container of
        Nothing -> 
            case ckind of
                CKProvides -> let newContainer = Map.insert rn (i, 0, 0) container in
                              buildResourceMap rest newContainer 
                CKUses     -> let newContainer = Map.insert rn (0, i, 0) container in
                              buildResourceMap rest newContainer
                CKRequires -> let newContainer = Map.insert rn (0, 0, i) container in
                              buildResourceMap rest newContainer

        Just (proi, usei, reqi) ->
            case ckind of
                CKProvides -> let newContainer = Map.insert rn (proi+i, usei, reqi) container in
                              buildResourceMap rest newContainer
                CKUses     -> let newContainer = Map.insert rn (proi, usei+i, reqi) container in
                              buildResourceMap rest newContainer
                CKRequires -> let newContainer = Map.insert rn (proi, usei, reqi+i) container in
                              buildResourceMap rest newContainer

mergeRMap :: [REnvUnit] -> REnv -> REnv
mergeRMap [] renv = renv
mergeRMap ((rn, (proi, usei, reqi)) : rest) renv =
    case Map.lookup rn renv of
        Nothing -> 
                let newRenv = Map.insert rn (proi, usei, reqi) renv in
                mergeRMap rest newRenv
        Just (proi1, usei1, reqi1) -> 
                let newRenv = Map.insert rn (proi + proi1, usei + usei1, reqi + reqi1) renv in
                mergeRMap rest newRenv
...
buildCompMap ((IC cn clause_list):rest) container = 
        case Map.lookup cn container of
            Nothing    -> 
                    let newContainer = Map.insert cn (buildResourceMap clause_list) container in
                        buildCompMap rest newContainer
            Just renv  -> 
                    let thisREnv = buildResourceMap clause_list
...
add_ckind ckind [] container = container
...
elabIComp ((IC cn clause_list):rest) standardComponent =
       case elabClauses clause_list of
           (Right standardclause) ->
                  elabIComp rest ((IC cn standardclause):standardComponent)
           (Left err) -> Left err 
